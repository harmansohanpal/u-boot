...
static struct usb_endpoint_instance endpoint_instance[1];
static struct usb_interface_instance interface_instance[1];
static struct usb_alternate_instance alternate_instance[1];
/* Structure for ep0 urb */
struct urb *ep0_urb;
/* String Descriptor */
static u8 wstrLang[4] = {4, USB_DT_STRING, 0x9, 0x4};
static struct usb_string_descriptor *usbtty_string_table[NUM_STRINGS];

/*Serial Number to distinguish between the
 * USB devices connected to the host PC.*/
static char serial_number[16] = "000000000000000";

/* Global Variable to tell which mode the device is in.*/
static int end_of_dfu_mode;
/* end_of_dfu_mode
 * 1 means Reset from the host
 * 2 means That download has just completed and
 * is waiting to check for whether reset has been issued
 * 0 means the device is in normal dfu mode */

/* Indicate that some error occured in some external file
 * The device mode may not be supported in the selected USB module
 * In such a case the dfu mode would be aborted.*/
void dfu_mode_end(void)
{
	/* Stop with the DFU mode */
	end_of_dfu_mode = 1;
}
...
/* Main initialisation function for usb in dfu mode */
int drv_usbdfu_init(int i)
{
#ifdef CONFIG_CMD_NAND
/* Switch nandecc to hw bch8 mode before writing */
	int type = 2;
	printf("\n");
	ti81xx_nand_switch_ecc(NAND_ECC_HW, type);
#endif
	/* Do some basic low level initialisation for the USB module */
	udc_init(i);
	/* The initialistaion of DFU instances */
	dfu_init_instance();
	udc_startup_events(device_instance);
	udc_connect();
	/* Timer to come out of the dfu mode */
	static int timer;
	int flag = 0;
	while (!flag) {
		if (ctrlc()) {
			/* Abort by the user using ctrl + c */
			printf("\nAbort!!!\n");
			break;
		}
		if (device_instance->dfu_state == DFU_STATE_dfuERROR) {
			printf("\nERROR: Coming out of DFU mode\n");
			break;
		}
		udc_irq(); /* Interrupt for USB device mode */
		switch (end_of_dfu_mode) {
		case 0:
			/* No reset or normal DFU mode */
			timer = 0x2000; /* Reset the timer */
			break;
		case 1:
			/* Reset or some error forcing out of dfu mode */
			printf("\nComing out of DFU mode\n");
			flag = 1;
			break;
		case 2:
			/* Indiacates Completion of one download
			*  Checking if a reset has been issued by the host
			*  There is some time gap in the completion of the
			*  and the reset, so this timer and delay has been
			*  introduced. */
			timer--;
			udelay(50);
			if (timer == 0) {
				/* No reset from the host */
				printf("Either press ctrl+c to abort or"
					" Run another command from the host"
					"\nYou are still in DFU mode");
				end_of_dfu_mode = 0;
				timer = 0x2000; /* reset the timer */
				/* Still in DFU mode */
			}
...
#ifdef CONFIG_CMD_NAND
	/* Switch nandecc back to haridware hamming code */
	ti81xx_nand_switch_ecc(NAND_ECC_HW, 0);
#endif
	end_of_dfu_mode = 0; /* Reseting the variable for further use */
	musb_platform_deinit(i);/* Deinitialise the usb module */
	dealloc_urb();/* free the urb structure allocated for dfu */
	return 0;
}
...
static int dfu_trailer_matching(const struct uboot_dfu_trailer *trailer)
{
	if (trailer->magic != UBOOT_DFU_TRAILER_MAGIC ||
	    ...
	    trailer->product != CONFIG_USBD_PRODUCTID_DFU)
		return 0;
...
static struct part_info *get_partition_nand(int idx)
{
	...
	if (mtdparts_init()) {
		/* Error */
		/* mtdparts variable not initialised properly */
		device_instance->dfu_state = DFU_STATE_dfuERROR;
		device_instance->dfu_status = DFU_STATUS_errUNKNOWN;
		return NULL;
	}
	if (list_empty(&devices)) {
		/* Error */
		/* There is no NAND partition available for DFU download */
		device_instance->dfu_state = DFU_STATE_dfuERROR;
		device_instance->dfu_status = DFU_STATUS_errUNKNOWN;
		return NULL;
	}
...
#endif
/* The starting and the ending address for RAM available for DFU download.
 * In case the MACRO CONFIG_CMD_NAND has been defined,
 * it implies that the NAND is working for the device.
 * In such a case if the device is unable to find suitable NAND working,
 * the control is taken out of DFU mode without allowing the download
 * to RAM. In case the config is not defined,
 * there is provision to download the required image to RAM.
 * The size of available RAM can be changed by changing the start
 * and the end address of the memory provided.
 * NOTE: RAM download is just for TESTING Purpose.
 * It has nothing to do with the Device Upgrade */
#define LOAD_ADDR	DFU_LOAD_ADDRESS
#define END_ADDR	DFU_END_ADDRESS
#ifdef CONFIG_CMD_NAND
static int initialize_ds_nand(
		struct usb_device_instance *dev, struct dnload_state *ds)
{
	...
	if (ds->nand->erasesize > sizeof(ds->_buf)) {
		printf("*** Warning - NAND ERASESIZE "
				"bigger than static buffer\n");
...
#else
	rc = nand_write_skip_bad(
			ds->nand, ds->off, &size, (u_char *)ds->buf, 0);
#endif
	if (rc) {
		/* must come out of dfu mode */
		debug("Error writing\n");
...
#else
	rc = nand_read_skip_bad(ds->nand, ds->off, &ds->nand->erasesize,
					(u_char *)ds->buf);
#endif
	if (rc) {
		/* must come out of dfu mode */
...
#endif
static int handle_dnload(struct urb *urb, u_int16_t val,
				 u_int16_t len, int first)
{
	static int delay; /* Variable to shorten the progress bar */
	struct usb_device_instance *dev = urb->device;
	...
	if (len > CONFIG_USBD_DFU_XFER_SIZE) {
		...
		/* protocol violation : Exit dfu mode */
	...
	if (len == 0) {
		/* Zero length packet to indicate completion of a download */
		debug("zero-size write -> MANIFEST_SYNC ");
		dev->dfu_state = DFU_STATE_dfuMANIFEST_SYNC;
		printf("]"); /* End of download indicated by 0 length packet */
		printf("\nDownload Complete\n");
		end_of_dfu_mode = 2;
		/* cleanup */
		...
		case 0:
			...
			ds->ptr = ds->buf;
			break;
/* using static buffer for all alternate interfaces */
#if 0
		case 1:
			if (ds->ptr >
	...
	case 0:
		if (first) {
			end_of_dfu_mode = 0;
			printf("\nStarting DFU DOWNLOAD to RAM (0x%8p)\n[",
				LOAD_ADDR);
			ds->buf = LOAD_ADDR;
			ds->ptr = ds->buf;
		}
		...
		ds->ptr += len;
		if (ds->ptr + len > END_ADDR) {
			/* Size of image exceed the allocated memory for DFU*/
			printf("\nEnd of memory available for DFU reached\n ");
		...
		break;
/* Using static buffer for all the alternate interfaces */
#if 0
	case 1:
		if (first) {
	...
	default:
		...
		if (first) {
			...
			if (rc)
				return rc;
			end_of_dfu_mode = 0; /* In normal dfu mode */
			printf("\nStarting DFU DOWNLOAD to partition '%s'\n[",
				ds->part->name);
		}
		delay += 1;
		if (delay % 3 == 0)
		...
		/* check partition end */
		if (ds->off + (ds->ptr - ds->buf) >
				ds->part->offset + ds->part->size) {
			/*End of write partition reached
			* Some problem! Exit from dfu mode*/
			printf("End of write exceeds partition end\n");
			dev->dfu_state = DFU_STATE_dfuERROR;
			dev->dfu_status = DFU_STATUS_errADDRESS;
			return RET_STALL;
...
static int handle_upload(struct urb *urb,
				 u_int16_t val, u_int16_t len, int first)
{
	...
	case 0:
		if (first) {
			printf("Starting DFU Upload of RAM (0x%8p)\n",
				LOAD_ADDR);
			ds->ptr = ds->buf;
		}
	...
	default:
		...
		if (ds->ptr >= ds->buf + ds->nand->erasesize &&
			...
			if (rc)
				return -EINVAL;
			if (len > remain) {
				debug("copying another %u bytes ",
							 len - remain);
				memcpy(urb->buffer + remain,
						 ds->ptr, len - remain);
				ds->ptr += (len - remain);
...
static struct usb_device_qualifier_descriptor dfu_dev_qual_descriptor = {
	.bLength		= USB_DT_DEVICE_SIZE,
...
static void dfu_init_strings(struct usb_device_instance *dev)
{
	...
	usb_strings[STR_LANG] =
		(struct usb_string_descriptor *)wstrLang;
	usb_strings[DFU_STR_MANUFACTURER] =
			 create_usbstring(CONFIG_USBD_MANUFACTURER);
	usb_strings[DFU_STR_PRODUCT] =
			create_usbstring(CONFIG_USBD_PRODUCT_NAME);
	usb_strings[DFU_STR_SERIAL] =
			create_usbstring(serial_number);
...
#else
			strdesc =
				create_usbstring("Nand not supported");
#endif
		}
		if (!strdesc)
			continue;
...
{
	int i;
	memset(device_instance, 0,
			sizeof(struct usb_device_instance));
	...
	device_instance->dfu_dev_qual_desc = &dfu_dev_qual_descriptor;
	device_instance->dfu_cfg_desc = &dfu_cfg_descriptor;
	device_instance->dfu_state = DFU_STATE_dfuIDLE;
	device_instance->device_descriptor = &dfu_dev_descriptor;
	device_instance->event = dfu_event;
	device_instance->dfu_status = DFU_STATUS_OK;

/*	if (system_dfu_state)
		printf("SURPRISE: system_dfu_state is already set\n");
*/
	system_dfu_state = &device_instance->dfu_state;
	dfu_init_strings(device_instance);
	...
	config_instance->configuration_descriptor = &(dfu_cfg_descriptor.ucfg);
	config_instance->interface_instance_array = interface_instance;

	/* Interface Instance */
	memset(interface_instance, 0,
		sizeof(struct usb_interface_instance));
	interface_instance->alternates = DFU_NUM_ALTERNATES;
	interface_instance->alternates_instance_array = alternate_instance;

	/* alternates instance */
	memset(alternate_instance, 0,
		sizeof(struct usb_alternate_instance));
	alternate_instance->interface_descriptor = dfu_cfg_descriptor.uif;
	alternate_instance->endpoints = 1;
	       ...
	       usb_device_event_t event, int data)
{
	...
	case DEVICE_RESET:
		...
		case DFU_STATE_dfuMANIFEST_WAIT_RST:
			device->dfu_state = DFU_STATE_dfuIDLE;
			/* Indicating reset from the host */
			end_of_dfu_mode = 1;
			break;
		default:
			break;
